import { Check, ListBullet } from "@medusajs/icons";
...
>(
  ...
  return (
    ...
    >
      ...
      {!isMulti && (
        <div className="ml-xsmall w-5">{isSelected && <Check size={16} />}</div>
      )}
    </div>
  );
};
...
const CheckboxAdornment = ({
  ...
  return (
    ...
    >
      <span className="self-center">{isSelected && <Check size={10} />}</span>
    </div>
  );
};
